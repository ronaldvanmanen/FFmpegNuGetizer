...
RuntimePackageName="$LibraryName.runtime.$LibraryRuntime"

RuntimePackageBuildDir="$BuildRoot/$RuntimePackageName.nupkg"

echo "$ScriptName: Producing $LibraryName runtime package folder structure in $RuntimePackageBuildDir..."
...
mkdir -p "$RuntimePackageBuildDir/runtimes/$LibraryRuntime/native" && cp -d "$InstallDir/lib/"*"so"* $_

echo "$ScriptName: Building $LibraryName runtime package (using NuGet $NuGetVersion)..."
nuget pack "$RuntimePackageBuildDir/$RuntimePackageName.nuspec" -Properties "version=$PackageVersion" -OutputDirectory $PackagesDir
LAST_EXITCODE=$?
if [ $LAST_EXITCODE != 0 ]; then
